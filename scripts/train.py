...
def main() -> None:
    ...
    val_loader = DataLoader(
        IsolatedWordDataset(val_samples),
        batch_size=int(config['train']['batch_size']),
        shuffle=False,
        num_workers=int(config['train']['num_workers']),
    )
    test_loader = DataLoader(
        IsolatedWordDataset(test_samples),
        batch_size=int(config['train']['batch_size']),
        shuffle=False,
        num_workers=int(config['train']['num_workers']),
    )
    ...
    if config['train'].get('scheduler') == 'multisteplr':
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer,
            milestones=config['train'].get('milestone_epochs', [20, 35]),
            gamma=float(config['train'].get('gamma', 0.2)),
        )
    elif config['train'].get('scheduler') == 'cosine':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=int(config['train']['epochs']),
            eta_min=float(config['train'].get('min_lr', 1e-5)),
        )
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(row)
        print(
            f"Epoch {epoch:03d} | train loss {train_metrics['loss']:.4f} acc {train_metrics['accuracy']:.4f} | "
